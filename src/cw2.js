...
const BizzAIChat = {
  init({
    ...
    width = "320px",
    height = "400px",
    apiUrl = "https://mmfast-339362962237.asia-south1.run.app",
  }) {
    ...
    var userId = localStorage.getItem("bizzai-user-id");

    var messageHistory = JSON.parse(
      localStorage.getItem("bizzai-history") || "[]"
    );
    let snapshotKeyHistory =
      JSON.parse(localStorage.getItem("bizzai-snapshot-key")) || [];
    ...
    function saveSnapshotKey(key) {
      snapshotKeyHistory.push(key);
      localStorage.setItem(
        "bizzai-snapshot-key",
        JSON.stringify(snapshotKeyHistory)
      );
      //   localStorage.setItem("bizzai-snapshot-key", snapshotKeyHistory);
    }
